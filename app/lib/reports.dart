...
class _DashboardSectionState extends State<DashboardSection> {
  ...
  @override
  void didUpdateWidget(DashboardSection old) {
    super.didUpdateWidget(old);
    if (old.api.workspace != widget.api.workspace) {
      data = null;
      load();
    }
  }
    ...
    try {
      final result = Map<String, dynamic>.from(
        await widget.api.json(
          'GET',
          widget.api.scoped('/dashboard?month=$month'),
        ) as Map,
      );
      if (mounted &&
          request == generation &&
...
class ReportsPage extends StatefulWidget {
  ...
  final String? initialProjectId, initialProjectName, initialEntryType;
  final String? initialFromDate, initialToDate;
  final bool lifetime;
  const ReportsPage({
    super.key,
    required this.api,
...
class _ReportsPageState extends State<ReportsPage> {
  ...
  void initState() {
    ...
    selectedProjectName = widget.initialProjectName;
    entryType = widget.initialEntryType;
    if (widget.lifetime) {
      fromDate = '0001-01-01';
      toDate = '9999-12-31';
    }
    fromDate = widget.initialFromDate ?? fromDate;
    toDate = widget.initialToDate ?? toDate;
    load();
  }

  @override
  void didUpdateWidget(ReportsPage old) {
    super.didUpdateWidget(old);
    if (old.api.workspace != widget.api.workspace) {
      summary = null;
      items = [];
  ...
  Future<void> load() async {
    ...
    try {
      final result =
          await widget.api.json('GET', widget.api.scoped(path)) as Map;
      if (mounted &&
          request == generation &&
          workspace == widget.api.workspace) {
        setState(() {
          summary = Map<String, dynamic>.from(result['summary'] as Map);
          items = m6Rows(result['items']);
  ...
  Future<void> date(bool start) async {
    ...
    change(() {
      if (start) {
        fromDate = value;
        if (fromDate.compareTo(toDate) > 0) toDate = value;
      } else {
        toDate = value;
        if (toDate.compareTo(fromDate) < 0) fromDate = value;
      }
    });
  }
            ...
            if (kind == 0) ...[
              Wrap(
                ...
                children: [
                  metric(
                    loc.income,
                    summary!['recordedIncome'],
                    onTap: () => change(() => entryType = 'INCOME'),
                  ),
                  metric(
                    loc.expense,
                    summary!['recordedExpenses'],
                    onTap: () => change(() => entryType = 'EXPENSE'),
                  ),
                  metric(
                    loc.m7RecordedDifference,
                    summary!['recordedDifference'],
                    onTap: () => change(() => entryType = null),
                  ),
                ],
              ),
              Text(
                loc.m7CurrentClassification,
                style: Theme.of(context).textTheme.bodySmall,
              ),
              for (final group in m6Rows(summary!['groups']))
                ListTile(
                  title: Text(
                    group['generalExpense'] == true
                        ? loc.m7GeneralExpenses
                        : '${group['equipmentName']}',
                  ),
                  subtitle: Text(
                  ...
                  onTap: () => change(() {
                    ...
                    generalExpense = group['generalExpense'] == true;
              ...
              Wrap(
                ...
                children: [
                  ...
                  metric(
                    loc.m7ExpenseRefunds,
                    summary!['expenseRefunds'],
                    onTap: () => change(() {
                      entryType = 'EXPENSE';
                      movementType = 'REFUND';
                    }),
            ...
            for (final item in items)
              Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                    kind == 1
                        ? '${localizedDate(context, item['movementDate'] as String?)} • ${item['movementType'] == 'REFUND' ? loc.m7Refund : loc.m7Settlement}${item['amount'] != item['movementTotal'] ? ' • ${loc.m7EquipmentShare}: ${localizedMoney(context, item['amount'])} • ${loc.m7OriginalMovement}: ${localizedMoney(context, item['movementTotal'])}' : ''}'
...
class ProjectSearchPicker extends StatefulWidget {
  final Api api;
  final String? selectedId, selectedName;
  final bool activeOnly, includeArchived, allowNone;
  const ProjectSearchPicker({
    super.key,
    required this.api,
    this.selectedId,
    this.selectedName,
  ...
  @override
  State<ProjectSearchPicker> createState() => _ProjectFilterPickerState();
}

class _ProjectFilterPickerState extends State<ProjectSearchPicker> {
  final search = TextEditingController();
  List<Map<String, dynamic>> items = [];
  bool archived = false, loading = true, loadingMore = false, hasMore = false;
  String? error;
  String? selectedName;
  int generation = 0, page = 0;
  Timer? debounce;
  @override
  void initState() {
    super.initState();
  ...
  Future<void> loadSelection() async {
    if (widget.selectedId == null || selectedName != null) return;
    final workspace = widget.api.workspace;
    try {
      final row = await widget.api.json(
        'GET',
        widget.api.scoped('/projects/${widget.selectedId}'),
      ) as Map;
      if (mounted && workspace == widget.api.workspace) {
        setState(() => selectedName = '${row['name']}');
      }
    } catch (_) {
      // A stale or unauthorized project must never be shown by its ID.
    }
  }

  Future<void> load({bool more = false}) async {
    if (more && (loadingMore || !hasMore)) return;
    final request = ++generation, workspace = widget.api.workspace;
    final requestedPage = more ? page + 1 : 0;
    setState(() {
      if (more) {
        loadingMore = true;
      } else {
    ...
    try {
      final query = Uri(
        queryParameters: {
          ...
          if (widget.activeOnly) 'status': 'ACTIVE',
          'page': '$requestedPage',
        },
      ).query;
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    content: SizedBox(
      ...
      child: Column(
        children: [
          TextField(
            key: const Key('projectPickerSearch'),
            controller: search,
            maxLength: 100,
            decoration: InputDecoration(labelText: l10n(context).uiSearch),
                ...
                : ListView(
                    children: [
                      ...
                      if (widget.selectedId != null &&
                        ...
                        ListTile(
                          key: const Key('projectPickerSelection'),
                      ...
                      if (items.isEmpty)
                        ListTile(title: Text(l10n(context).m6NoProjects)),
                      for (final item in items)
                        ListTile(
                      ...
                      if (hasMore)
                        TextButton(
                          key: const Key('projectPickerMore'),
                          onPressed: loadingMore
                              ? null
                              : () => load(more: true),
                          child: loadingMore
                              ? const CircularProgressIndicator()
                              : Text(l10n(context).next),
                        ),
                    ],
                  ),
          ),
...
String _localizedMonth(BuildContext context, String month) {
  ...
  return DateFormat.yMMMM(Localizations.localeOf(context).languageCode)
      .format(DateTime(parts[0], parts[1]));
}
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text(l10n(context).m7ChooseMonth),
      content: SizedBox(
        width: 360,
        height: 330,
        child: Column(
                ...
                children: [
                  for (var m = 1; m <= 12; m++)
                    TextButton(
                      ...
                      child: Text(
                        DateFormat.MMMM(language).format(DateTime(year, m)),
                      ),
                    ),
                ],
              ),
