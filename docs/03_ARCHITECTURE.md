...
Status: BASELINE for broader architecture; current M0–M6 implementation note (2026-09-26): Flyway V18–V21 adds workspace-owned `organization`, `equipment_organization_assignment`, a unified typed `project_or_contract` (PROJECT/CONTRACT), `project_equipment_link`, optional `financial_entry.project_id`, project attachments, and selected-organization membership/invitation associations. Organizations are groupings, never tenants; equipment and project association remain optional. Partial unique indexes cap current assignment and active project/equipment link, with row locks around transitions. Existing equipment-scoped access uses membership scope dynamically against current equipment assignment on each request. A project is a classification, not a source of financial truth: exact summary values derive from posted M2 entries and their settlements/refunds. Project detail and attachments require PROJECT_VIEW/MANAGE plus project scope; financial summary additionally requires FINANCE_VIEW. No separate contracts subsystem, stored project money, or M6 reminder scheduler.

M7 read-side boundary (approved scope, 2026-09-26): recorded entries aggregate original posted totals by operation date; movement report aggregates settlement/refund identities by each movement date; current outstanding uses lifetime M2 net settlements across all operation dates. Report access requires REPORT_VIEW and FINANCE_VIEW with current equipment/organization/project scope applied before totals and pagination. Dashboard finance cards use FINANCE_VIEW and remain separate from report permission. These are operational views over M2, not a ledger, profit calculation, or stored financial snapshot. See D-27 and `docs/tasks/007_M7_STATUS.md` for implementation state.

M4 local implementation and limits are described in HANDOFF.
...
M5 implementation note (2026-09-25): membership is the per-request workspace authorization source. `Access` evaluates active membership, capabilities, equipment scope and financial mode across M0–M4 routes as well as team routes. SELECTED scope hides shared originals unless all allocations are visible, and general expenses require all-equipment scope. A PostgreSQL advisory workspace lock serializes driver assignment changes; partial unique indexes prevent two active assignments per driver or equipment. Financial submissions are separate from M2 totals; approval locks the submission, creates one M2 expense and links the existing receipt metadata. V14–V16 implement the schema and owner constraints. V17 adds a unique self-reference for a new request created from a rejected one; the original and any approved request remain immutable review history. New attachment metadata can reference the same immutable object bytes. Production identity, object storage and Push adapters remain separate launch work.
