...
تحديث 2026-09-24: اكتملت محليًا شريحة إيراد كامل/جزئي/غير مستلم مع تحصيلات متعددة مؤرخة في الأصل نفسه، بعد شريحة المصروف. التحقق: 16 اختبار backend على PostgreSQL، 9 اختبارات Flutter، اختبار تكامل iOS مع API/PostgreSQL، بناء ويب، وفحص واجهة ويب حي. التفاصيل والحدود في [M2_INCOME_SETTLEMENTS](evidence/M2_INCOME_SETTLEMENTS.md). الفرع `work/m0-m1`؛ التزام هذه الشريحة موضح في `git log -1` بعد الحفظ. بقية M2 وV1 غير مكتملتين. التالي: شريحة محدودة من بقية M2 (تعديل العملية المسموح أو بحث/تصفية السجل) مع تحديد شروط القبول قبل التنفيذ. تغييرات `AGENTS.md` و`.codex/` السابقة حُفظت خارج هذا الالتزام، ولم يُستخدم وكيل فرعي.

تحديث 2026-09-24: بدأ M2 بتكليف المالك على الفرع `work/m0-m1`، والتزام الشريحة `95590f9`. اكتملت محليًا شريحة مصروف جزئي/غير مدفوع وتسويات متعددة مؤرخة. تحقق ويب فعلي بعرض ضيق وعريض، وفحص iOS simulator موسع لمسار الجزئي والتسوية اللاحقة. كشف اختبار iOS خلل عمر حقل دفعة وأُصلح، ثم نجح الاختبار والتحليل وبناء الويب. التفاصيل والأوامر والحدود في [M2_EXPENSE_SETTLEMENTS](evidence/M2_EXPENSE_SETTLEMENTS.md). M2 بكاملها وV1 غير مكتملتين. التعديلات السابقة غير الملتزم بها في `AGENTS.md` و`.codex/` تخص سياسة الاستهلاك وحُفظت دون تغيير في هذه المهمة. لم يُستخدم وكيل فرعي.

التسليم السابق: **المهمة 001 مكتملة كشريحة M0/M1 محلية؛ V1 غير مكتمل.** ما يلي يصف دليل تلك الشريحة قبل إضافة M2.
...
## التالي — توصية فقط
شريحة المصروف الجزئي/غير المدفوع والدفعات المؤرخة منفذة ومتحقق منها محليًا على ويب ضيق وعريض ومحاكي iOS كما في الدليل الجديد أعلاه. التالي: مهمة دخل جزئي/غير مستلم مستقلة، ثم بقية M2 والتخزين الخاص بالترتيب. قرارات OPEN_QUESTIONS لم تُحسم خفية. لا عمل أو مراقبة مستمرة بعد انتهاء هذه الجولة.
