# حالة التسليم — M0/M1
تحديث 2026-09-24: بدأ M2 بتكليف المالك على الفرع `work/m0-m1`، والتزام الشريحة `95590f9`. اكتملت محليًا شريحة مصروف جزئي/غير مدفوع وتسويات متعددة مؤرخة. تحقق إضافي على ويب فعلي بعرض ضيق: جزئي ثم ثلاث دفعات ورفض الزائد واسترجاع بعد تحديث الصفحة، وغير مدفوع بلا دفعة مع إلزامية الطرف. نجح اختبار iOS simulator لمسار M1 بعد التعديل. التفاصيل والأوامر والحدود في [M2_EXPENSE_SETTLEMENTS](evidence/M2_EXPENSE_SETTLEMENTS.md). M2 بكاملها وV1 غير مكتملتين. التعديلات السابقة غير الملتزم بها في `AGENTS.md` و`.codex/` تخص سياسة الاستهلاك وحُفظت دون تغيير في هذه المهمة. لم يُستخدم وكيل فرعي.
...
## التالي — توصية فقط
شريحة المصروف الجزئي/غير المدفوع والدفعات المؤرخة منفذة ومتحقق منها محليًا على ويب ضيق كما في الدليل الجديد أعلاه. التالي: فحص عرض سطح المكتب ومسار M2 على iOS عند الحاجة، ثم مهمة دخل جزئي/غير مستلم مستقلة، ثم بقية M2 والتخزين الخاص بالترتيب. قرارات OPEN_QUESTIONS لم تُحسم خفية. لا عمل أو مراقبة مستمرة بعد انتهاء هذه الجولة.
