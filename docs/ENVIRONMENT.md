...
| PostgreSQL / test database | PostgreSQL 17.11 (Debian 17.11-1.pgdg13+2), aarch64; dev/test containers healthy at latest check | `docker compose -f infra/compose.yaml --profile test ps`, `psql ... select version()`, backend tests | Isolated databases on loopback 55432/55433; Flyway V1–V21 validated on isolated test DB; existing dev DB upgraded from V17 to V21 without changing the prior equipment/financial-entry counts |
...
## Current validation checkpoint — 2026-09-26
- Backend: Boot 4.1.1 targets Java 21, real PostgreSQL 17.11; Flyway V1–V21 validated on isolated test DB. Existing dev DB upgraded V17 → V21 and its prior 30 equipment / 49 financial entries remained present; 81 tests / 0 failures / 0 errors / 0 skipped in the latest run.
- Flutter: analyze PASS; 86 unit/widget tests PASS; M6 release web build PASS. Connected M6 API journey against local PostgreSQL PASS. Earlier M3 iOS simulator integration 1/1 PASS (M2 prior 2/2); M6 iOS and Android not run.
...
- `JAVA_HOME=$(/usr/libexec/java_home -v 21) ./scripts/backend-dev.sh` from the repository root starts the current source on `127.0.0.1:8080` and applies pending Flyway migrations. Stop or restart an older API process before relying on current M6 behavior.
