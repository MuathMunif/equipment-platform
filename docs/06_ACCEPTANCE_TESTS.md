...
M5 local checkpoint (2026-09-25): `TeamM5Test` adds 16 backend integration tests on PostgreSQL, with 72 total backend tests; 77 Flutter widget/unit tests total. Tests cover invitation lifecycle, owner protection, active driver uniqueness/history/concurrency, REVIEW bypass attempts through legacy M2 routes, double approval, revocation, selected-scope resource/file/totals denial, and workspace switching. A connected web smoke covered invitation acceptance and assignment; the complete M5 journey and iOS/Android remain unverified.
