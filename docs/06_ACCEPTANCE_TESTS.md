...
- REPORT-08 (M7 final acceptance): project figures open the matching report basis and project filter, with lifetime dates for recorded/net movement figures and no month on current outstanding; Back preserves the project. Project selection supports server search, bounded pagination past 100 accessible projects, and an authorized existing selection outside the first page. Backend and widget regressions cover navigation, page/scope and refund net semantics; complete populated browser journey remains a manual gate.
