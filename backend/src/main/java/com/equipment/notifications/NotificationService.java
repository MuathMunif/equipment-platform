...
public class NotificationService {
    ...
    private VisibleNotifications visible(Actor actor,UUID workspace){
        ...
        String scope=m.scope().equals("ALL_EQUIPMENT")?"true":m.scope().equals("ASSIGNED_EQUIPMENT")?"exists(select 1 from driver_assignment x where x.workspace_id=n.workspace_id and x.driver_user_id=? and x.equipment_id=("+equipment+") and x.ended_at is null)":m.scope().equals("SELECTED_ORGANIZATIONS")?"exists(select 1 from equipment_organization_assignment x join membership_organization mo on mo.workspace_id=x.workspace_id and mo.organization_id=x.organization_id where x.workspace_id=n.workspace_id and x.equipment_id=("+equipment+") and x.ended_at is null and mo.user_id=?)":"exists(select 1 from membership_equipment x where x.workspace_id=n.workspace_id and x.user_id=? and x.equipment_id=("+equipment+"))";
