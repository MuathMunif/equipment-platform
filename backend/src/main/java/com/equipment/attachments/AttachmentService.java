...
public class AttachmentService {
    ...
    public record Attachment(UUID id,UUID entryId,UUID documentVersionId,UUID issueId,UUID maintenanceId,UUID projectId,String filename,String mediaType,long size,String state,String scanStatus) {}
    ...
    public List<Attachment> listProject(Actor actor,UUID w,UUID project){access.project(actor,w,project,"PROJECT_VIEW");return db.queryForList("select * from attachment where workspace_id=? and project_id=? order by created_at,id",w,project).stream().map(this::view).toList();}
    @Transactional public Attachment initiateProject(Actor actor,UUID w,UUID project,String key,Initiate request){access.project(actor,w,project,"PROJECT_MANAGE");String filename=Values.text(request.filename(),200,"اسم الملف");if(filename.contains("/")||filename.contains("\\")||filename.chars().anyMatch(c->c<32||c==127))throw ApiException.invalid("اسم الملف غير صالح");if(request.mediaType()==null||!ContentValidator.TYPES.contains(request.mediaType()))throw new ApiException(400,"UNSUPPORTED_FILE","صيغة الملف غير مدعومة");if(request.size()==null||request.size()<=0||request.size()>ContentValidator.MAX_BYTES)throw new ApiException(413,"FILE_TOO_LARGE","الملف أكبر من الحد المسموح");UUID id=retries.execute(w,actor.userId(),"project.attachment.create:"+project,key,Values.payload(filename,request.mediaType(),request.size()),()->{var parent=db.queryForList("select archived_at from project_or_contract where workspace_id=? and id=? for update",w,project);if(parent.isEmpty()||parent.getFirst().get("archived_at")!=null)throw ApiException.missing();Long count=db.queryForObject("select count(*) from attachment where workspace_id=? and project_id=?",Long.class,w,project);if(count>=10)throw new ApiException(409,"ATTACHMENT_LIMIT","الحد 10 مرفقات");UUID created=UUID.randomUUID();db.update("insert into attachment(id,workspace_id,project_id,original_name,declared_type,declared_size,state,created_by) values(?,?,?,?,?,?,'PENDING',?)",created,w,project,filename,request.mediaType(),request.size(),actor.userId());audit.record(w,actor.userId(),"PROJECT_ATTACHMENT_ADDED",created);return created;});return view(require(w,id,false));}
    ...
    public Attachment upload(Actor actor,UUID workspace,UUID id,String mediaType,byte[] bytes) {
        ...
        if(row.get("project_id")!=null){var parent=db.queryForList("select archived_at from project_or_contract where workspace_id=? and id=? for update",workspace,row.get("project_id"));if(parent.isEmpty()||parent.getFirst().get("archived_at")!=null)throw new ApiException(409,"PROJECT_ARCHIVED","المشروع مؤرشف");}
        try {
