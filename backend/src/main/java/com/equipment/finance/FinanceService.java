...
public class FinanceService {
    ...
    public record Create(UUID equipmentId,tools.jackson.databind.JsonNode amount,String category,String operationDate,String paidOn,String note,String paymentStatus,tools.jackson.databind.JsonNode initialPaid,String partyName,String dueDate) {}
    ...
    public record Entry(UUID id,UUID equipmentId,String equipmentName,String amount,String currency,String category,String operationDate,String note,String lifecycle,String paid,String remaining,String settlementStatus,String partyName,String dueDate,String createdAt,List<Settlement> settlements) {}
    ...
    public Entry create(Actor actor,UUID workspace,String key,Create request) {
        ...
        if(!Set.of("FULL","PARTIAL","UNPAID").contains(status)) throw ApiException.invalid("اختر حالة الدفع");
        BigDecimal initial;
        if(status.equals("FULL")) {
            ...
            if(request.initialPaid()==null || !request.initialPaid().isString()) throw ApiException.invalid("حدد المبلغ المدفوع أولًا");
            initial=Values.money(request.initialPaid().asString());
            if(initial.compareTo(amount)>=0) throw ApiException.invalid("الدفعة الأولى يجب أن تقل عن الإجمالي");
        } else {
            if(request.initialPaid()!=null || request.paidOn()!=null) throw ApiException.invalid("لا توجد دفعة عند اختيار غير مدفوع");
        ...
        String payload=request.paymentStatus()==null
        ...
        UUID id=retries.execute(workspace,actor.userId(),"expense.create",key,payload,()->{
            UUID created=UUID.randomUUID();
            db.update("insert into financial_entry(id,workspace_id,equipment_id,amount,currency,category,operation_date,note,lifecycle,created_by,party_name,due_date) values(?,?,?,?,'SAR',?,?,?,'POSTED',?,?,?)",created,workspace,request.equipmentId(),amount,category,java.sql.Date.valueOf(date),note,actor.userId(),party,due==null?null:java.sql.Date.valueOf(due));
            ...
            audit.record(workspace,actor.userId(),initial.compareTo(amount)==0?"PAID_EXPENSE_CREATED":"EXPENSE_CREATED",created); return created;
    ...
    public Entry settle(Actor actor,UUID workspace,UUID entryId,String key,AddSettlement request) {
        ...
        retries.execute(workspace,actor.userId(),"expense.settle",key,Values.payload(entryId,amount,paidOn),()->{
            ...
            if(amount.compareTo(total.subtract(paid))>0) throw ApiException.invalid("الدفعة أكبر من المتبقي");
            ...
            audit.record(workspace,actor.userId(),"EXPENSE_SETTLED",entryId);
            return settlementId;
        });
