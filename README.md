...
تطبيق Flutter عربي RTL وباك إند Spring Boot وPostgreSQL فعلي. اكتملت M0/M1 محليًا ونُفذت ثلاث شرائح من M2؛ بقية M2 وV1 غير مكتملتين، ولا يوجد إعداد إنتاج.

المسار المنفذ: دخول تطويري ← مساحة مالك تلقائية ← معدة بالاسم والموديل ← مصروف أو إيراد ← دفعات أو تحصيلات مؤرخة داخل العملية نفسها ← مرفق اختياري ← تعديل موثق أو إلغاء قيد خطأ مع سبب ← الرجوع للسجل والملف بعد التحديث. لا تحتاج مؤسسة أو فريقًا. المصروف كامل/جزئي/غير مدفوع، والإيراد مستلم كاملًا/جزئيًا/غير مستلم. يُطلب اسم الطرف عند وجود متبقٍ، وموعد الاستحقاق اختياري عند وجوده. المبلغ يُنقل كنص عشري ويحسب بـBigDecimal/PostgreSQL NUMERIC؛ التسوية والمرفق لا ينشئان عملية مالية ثانية.
...
آخر تشغيل لهذه الشريحة: 11 اختبار Flutter ناجحًا. تشمل المدخلات العربية الدقيقة، العرضين الضيق والواسع، الاسم المطلوب، مهلة HTTP كاملة، بقاء مفتاح الحفظ بعد فقد الرد، نموذجي المصروف والإيراد الجزئيين، تعديل العملية، تأكيد الإلغاء وعرض الحالة الملغاة، واستبدال مرفق مرفوض دون إنشاء عملية مالية جديدة. اختبارات widget تستخدم HTTP doubles ولا يُقدَّم نجاحها كدليل تكامل فعلي؛ اختبار iOS مع API/PostgreSQL وفحوص الويب الحية السابقة موثقة في HANDOFF والأدلة.
...
- تدفقات المصروف والإيراد الكاملة/الجزئية/غير المسددة وتسوياتها وتعديلها وإلغاء الخطأ محفوظة محليًا. تعديل الإجمالي لا يقل عن مجموع التسويات؛ الإلغاء يبقي التسويات والمرفقات في التاريخ ويمنع التعديل والتسوية الجديدة. الإلغاء ليس استردادًا للمال. لا توجد حاليًا واجهة مجاميع مالية نشطة؛ أي مجاميع مستقبلية يجب أن تستبعد `CANCELLED`. البحث/التصفية المالية الموسعة، مسودة الصورة واستكمالها، التخزين الخاص المربوط بـS3، والمراجعة/التقارير لم تكتمل. المصروف العام/المشترك، الاسترداد، الأرشفة، الفريق، والتنبيهات غير منفذة. لا GPS أو OCR أو بوابة دفع.
